...
public class SemillaTest {
	...
	public void construirSemilla() {
		...
		Integer[][] matrizDeAdyacencia = new Integer[][] {
				new Integer[] { 0, 1, 1 }, new Integer[] { 1, 0, 1 },
				new Integer[] { 1, 1, 0 } };

		// Crear nueva semilla
		Semilla semilla = new Semilla(Semilla.recorridoEnProfunidad, nNodos,
				esDirigido, matrizDeAdyacencia);

		// El tipo de pregunta debe ser 0
		assertEquals(semilla.getTipoPregunta(), (Integer) 0);

		// El n�mero de nodos debe ser 3
	...
	public void recuperarSemilla() {
		...
		// Recuperar la semilla a partir de su c�digo
		try {
			Semilla semilla = new Semilla(codigoSemilla);

			assertEquals(semilla.getTipoPregunta(),
					Semilla.recorridoEnProfunidad);
			assertEquals(semilla.getNNodos(), (Integer) 4);
			assertEquals(semilla.esDirigido(), false);

			// La matriz de adyacencia debe ser
	...
	public void recuperarSemillaErronea() {
		// Valor de la pos. 0 (correspondiente al Tipo de Pregunta) demasiado
		// alto
		try {
		...
		// probarlo,
		// pues su rango es [0~9] por lo que cualquier valor entero ser�
		// correcto

		// El valor de la pos. 2, correspondiente al booleano que indica si es
		// dirigido, no
		// es necesario probarlo, pues el programa simplemente entiende como
		// positivo cualquier
